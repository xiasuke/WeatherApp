...
class OwmApiClient(object):
    ...
    def get_forecast(self, city_id):
        ...
        new_forecast_list = self.organize_forecast(forecast_results["list"])
        forecast_results["list"] = new_forecast_list
    ...
    def organize_forecast(self, forecast_list):
        new_forecast_list = []
        pre_date = None
        day_forecast = []
        for forecast in forecast_list:
            date = forecast["dt_txt"][0:10]
            if pre_date is None or pre_date == date:
                pre_date = date
                day_forecast.append(forecast)
            elif pre_date != date:
                pre_date = date
                new_forecast_list.append(day_forecast)
                day_forecast = [forecast]

        # print new_forecast_list
        return new_forecast_list
